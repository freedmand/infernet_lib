import {Time, TimeDuration, createTimeClass} from './temporal.js';
...
test('create time class', t => {
  const Day = createTimeClass('Day', (text) => {
    const [year, month, day] = text.split('-');
    return Date.UTC(year, parseInt(month) - 1, day);
  }, 'day');
  const time = new Day('2016-03-01');
  t.is(time.year(), 2016);
  t.is(time.month(), 2);
  t.is(time.dayOfMonth(), 1);
  t.is(time.dayOfYear(), 61);
  t.is(time.dayOfWeek(), 2);
  t.is(time.resolution, 'day');
  t.throws(() => time.hour());
});