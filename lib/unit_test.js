...
test('unit does not eq', t => {
  t.false(distanceTable.value(6, 'ft').eq(distanceTable.value(1.9, 'yd')));
  t.false(distanceTable.value(2640, 'ft').eq(distanceTable.value(0.6, 'mi')));
  t.false(distanceTable.value(1000, 'in').eq(distanceTable.value(254001, 'mm')));
});

test('unit neq', t => {
  t.true(distanceTable.value(6, 'ft').neq(distanceTable.value(1.9, 'yd')));
  t.true(distanceTable.value(2640, 'ft').neq(distanceTable.value(0.6, 'mi')));
  t.true(distanceTable.value(1000, 'in').neq(distanceTable.value(254001, 'mm')));
});
...
test('unit sub', t => {
  t.true(distanceTable.value(2, 'km').sub(distanceTable.value(2, 'm')).eq(distanceTable.value(1998, 'm')));
});

test('unit mul', t => {
  t.true(distanceTable.value(2, 'km').mul(new Float(4)).eq(distanceTable.value(8000, 'm')));
});