...
export class Dataset {
  ...
  toVegaEncoding(columnName, binIfQuantitative = false) {
    ...
    if (binIfQuantitative && series.typeSpec == 'quantitative') {
      spec.bin = true;
    }
    return spec;
  }
  ...
  toVegaHistogram(columnName, mark = 'bar', width = 500) {
    ...
    if (series.typeSpec == 'temporal') mark = 'line';

    let encoding = {};
    encoding[flip ? 'y' : 'x'] = this.toVegaEncoding(columnName, true);
    encoding[flip ? 'x' : 'y'] = {
      aggregate: 'count',
      type: 'quantitative',
    };
